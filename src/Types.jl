...
mutable struct Model
  ...
  mutable struct Workspace
    ...
    function Workspace(p::QOCS.Model,sm::ScaleMatrices,ci::QOCS.ChordalInfo)
      m = p.m
      n = p.n
      ws = new(p,sm,ci,zeros(n),zeros(m),zeros(m),zeros(m),0.,Float64[],Info([0.]),ResultTimes())
    ...
    function Settings(;
      ...
      obj_true_tol = 1e-3
        ...
        new(rho,sigma,alpha,eps_abs,eps_rel,eps_prim_inf,eps_dual_inf,max_iter,verbose,
          ...
          adaptive_rho_interval,adaptive_rho_tolerance,verbose_timing,RHO_MIN,RHO_MAX,RHO_TOL,decompose,complete_dual,time_limit,obj_true,obj_true_tol)
    end
  end
